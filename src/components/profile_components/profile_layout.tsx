...
export default function ProfileLayout({
    ...
    return (
        ...
        <main
            className={`relative w-full flex-1 h-screen lg:max-w-7xl lg:mx-auto px-5 md:px-14 lg:px-20 xl:px-0 flex flex-col `}
        >

            <Header />
            <main className="flex-1 max-h-full w-full  flex flex-row mt-10 gap-5">
         {
             pathname.includes("profile/update")?null:    <section className="flex flex-col gap-4 text-white text-sm w-48 my-4 mr-4 cursor-pointer">
               <div onClick={()=>{
                router.push("/profile")
