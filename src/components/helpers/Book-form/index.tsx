...
export default function BookFormComponent({
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { resetForm }) => {
            ...
            try {
                ...
                const result = await axiosPublic.post('/lms/book-trail', payload);
    ...
    return <section className='relative  mx-auto p-10 bg-white rounded-2xl w-96 md:h-80 lg:h-auto overflow-y-auto'>
          <img
            onClick={(e) => {
                closeModel(false)
            }}
            alt='cancel icon'
            className="cursor-pointer absolute text-blue bg-black rounded-full h-6 w-6 top-4 right-4"
            src={`${basePath}/cancel.png`} />
 
        <form   id='my-form' autoComplete="off" onSubmit={formik.handleSubmit}>
            <p className='text-3xl font-semibold'>Book Your Free <span className='text-blue'>Trial</span></p>
            <div className='mt-4 flex flex-row gap-8'>
                <div className='flex-1'>
                    <input
                        ...
                        className="block px-4 w-full border border-black rounded-lg bg-white h-14 text-black shadow-sm ring-gray-300 placeholder:font-medium placeholder:text-gray-400 placeholder:pl-3 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6"
                    />
                    {formik.errors.fullName ? (
                        <div className="text-sm text-red mt-2 ml-2">{formik.errors.fullName}</div>
                    ) : null}
                </div>
            ...
            <div className='mt-4 flex flex-row gap-8'>
                <div className='flex-1'>
                    ...
                    {formik.errors.mobile ? (
                        <div className="text-sm text-red mt-2 ml-2">{formik.errors.mobile}</div>
                    ) : null}
                </div>
            </div>
            <div className='mt-4 flex flex-row gap-8'>
            ...
            <button form='my-form' type='submit' className="mt-4 flex w-full justify-center rounded bg-blue p-3 font-medium text-black ">
                {
