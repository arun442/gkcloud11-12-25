...
console.log(courseList)
    ...
    return (
        <Popover className="relative">
            <Popover.Button  className="inline-flex items-center mx-8 py-2 text-sm font-medium leading-6 text-white">
                <span>Programs</span>
            ...
            >
                <Popover.Panel className="absolute left-1/2 z-10 mt-5 flex w-screen max-w-max -translate-x-1/2 px-4">
                    <div className="w-screen max-w-4xl flex flex-row overflow-hidden rounded-3xl bg-white text-sm leading-6 shadow-lg ring-1 ring-gray-900/5">
                        <div className=" w-1/4">
                            {[{
                                partnerName: "All Courses"
                            }, ...partnerData].map((item) => (
                                <div onClick={(event)=>{
                                    setSelectedCat(item.partnerName);
if(item.partnerName=="All Courses"){
setCourse(courseList.current);
}else{
    ...
    setCourse(courseList.current.filter((e:any)=>e.partnerId==item.partnerId));
                                    ...
                                    <div className='flex-1'>
                                        <a className={classNames(" font-normal",selectedCat==item.partnerName?"text-blue":"text-gray-900")}>
                                            {item.partnerName}

                                        </a>

                                    </div>
                                    <ChevronRightIcon className={classNames("h-4 w-4 text-gray-600 font-normal group-hover:text-indigo-600",selectedCat==item.partnerName?"text-blue":"text-gray-600")} aria-hidden="true" />
