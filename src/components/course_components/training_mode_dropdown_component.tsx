...
export default function TrainingModeDropdown({data,setData}:{data:any,setData:any}) {
  ...
  return (
    <div className="px-1 py-1 bg-dark_blue  box-border border-blue border-2 rounded-full">
      <Menu as="div" className="relative inline-block text-left">
        ...
        >
          <Menu.Items className="absolute z-10 m-10 sm:mr-0 right-0 mt-2 w-full md:w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black/5 focus:outline-none">
          {
            trainingData.map((e, index) => <div key={index} className="px-1 py-1">
                <Menu.Item>
                  {({ active }) => (
                    <button
                    onClick={data=>{
                      ...
                      className={`${active ? 'bg-violet-500 text-blue' : 'text-gray-900'
                        } group flex w-full  items-start text-start rounded-md px-2 py-2 text-sm`}
                    >
